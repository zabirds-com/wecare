...
class DefaultDashboard extends Component {
    ...
    if (nextProps.clients !== this.props.clients) {
      ...
      clients.forEach(element => {
        if (element.family) {
          ...
          if (typeof (element.family) == "object") {
            family = element.family.label;
          }

          if (this.state.distinctFamilies.indexOf(family) < 0) {
            this.state.distinctFamilies.push(family);
          }
  ...
  render() {
    ...
    for (let i = 0; i < clients.length; i++) {
      if (clients[i].category === "") nonCategory++;
    }
    return (
      <React.Fragment>
        <div className="">
                ...
                <CardBody style={{ backgroundColor: "#f5f6f8" }}>
                  <Row>
                      ...
                      >
                        ...
                        <div>
                          {this.state.selectedArray.map((client, i) =>
                            ...
                            moment.utc(client.dob).format("DD-MM") ===
                    ...
                    >
                      ...
                      >
                        <h5>Next Contact Date > 7 Days</h5>
                        <div>
                          {this.state.selectedArray.map((client, i) =>
                            moment
                              .duration(curTime.diff(client.nextFollowUpDate))
                              .asDays() > 7 ? (
                                <Link
                                  style={{ display: "block" }}
                                  to={`/clients/${client._id}/view`}
                      ...
                      >
                        ...
                        <div>
                          {this.state.selectedArray.map((client, i) =>
                            moment
                              .duration(
                                moment(client.nextFollowUpDate).diff(curTime)
                              )
                              .asDays() <= 7 &&
                              moment
                                .duration(
                    ...
                    >
                      ...
                      >
                        <h5>
                          Number of Families: {" "}
                          {this.state.distinctFamilies.length}
                        </h5>
                      </div>
                    </Col>
                  </Row>
...
export default connect(mapStateToProps, { getClients, getUser })(
  DefaultDashboard
);
